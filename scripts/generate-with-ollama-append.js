...
/**
 * PrepGenie - FREE Question Generation with Ollama (APPEND MODE)
 *
 * This version APPENDS to existing question files instead of creating new ones.
 * Perfect for bulk generation + incremental updates.
 *
 * Usage:
...
async function generateQuestion(model, examName, subject, topic, difficulty, exam_id, subject_id) {
  const prompt = `Create a ${difficulty} ${examName} exam question about ${topic} in ${subject}.
...
Return ONLY this JSON (no extra text):
{
  ...
  "correct_answer": 2,
  "explanation": "detailed explanation with steps and reasoning"
}

Rules:
- correct_answer is 0, 1, 2, or 3
- Make all 4 options realistic
- Explanation should be 50+ words
- Question should be clear and specific`;
  ...
  try {
    ...
    return {
      ...parsed,
      difficulty,  // Use the passed difficulty parameter, not parsed
      exam_id,
      subject_id,
      topic,
      year: `AI Generated ${new Date().getFullYear()}`,
      source_detail: `AI-Generated Practice Question (${model})`,
...
// Parse existing CSV and return questions
...
function questionsToCSVRows(questions) {
  return questions.map(q => {
    const fields = [
      ...
      q.source_type || 'ai-generated', q.verified || false,
    ];
...
async function main() {
  console.log(`
╔═══════════════════════════════════════════════════════════════╗
║    PrepGenie - FREE Question Generation (APPEND MODE)         ║
╚═══════════════════════════════════════════════════════════════╝
`);
...
`);
  ...
  console.log(`\n🎉 Done! Run this script again anytime to add more questions.`);

#!/usr/bin/env python3
"""
Insert Phase 3.2: Synonyms & Antonyms (100Q)
"""
import psycopg2
...
def main():
    print("🚀 Inserting Phase 3.2: Synonyms & Antonyms (100Q)...\n")

    conn = psycopg2.connect(**conn_params)
    cur = conn.cursor()

    try:
        with open('output/synonyms-antonyms-100Q.sql', 'r', encoding='utf-8') as f:
            sql_content = f.read()

        cur.execute(sql_content)
        conn.commit()
        print("✅ Inserted 100 questions from synonyms-antonyms-100Q.sql\n")
        ...
        cur.execute("""
            ...
            WHERE topic_id = 'synonyms-antonyms'
        """)

        row = cur.fetchone()
        total, easy, medium, hard = row
        print(f"✅ synonyms-antonyms: {total}/100 questions (Easy: {easy}, Medium: {medium}, Hard: {hard})")
        ...
        if total == 100:
            print("🎉 SUCCESS! Synonyms & Antonyms insertion complete!\n")
        else:
            print(f"⚠️  Expected 100 questions, got {total}\n")

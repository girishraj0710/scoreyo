...
// Complete QUESTION-FORMATION - 29 more questions (to reach 60 total)
const QUESTION_FORMATION_FINAL_29 = [
  {
  ...
  {
    topic: 'question-formation', level: 'A2',
  ...
  {
    topic: 'question-formation', level: 'B1',
...
async function insertQuestions() {
  ...
  try {
    console.log(`\n🎓 QUESTION-FORMATION - Final 29 questions (complete to 60)`);
    console.log('='.repeat(80));

    let inserted = 0;
    for (const q of QUESTION_FORMATION_FINAL_29) {
      await pool.query(
      ...
      inserted++;
      if (inserted % 10 === 0) {
        console.log(`   ✓ Inserted ${inserted}/29...`);
      }
    }

    console.log(`\n✅ Inserted all ${inserted} questions!`);
    console.log(`📊 QUESTION-FORMATION now complete: 60/60 total\n`);

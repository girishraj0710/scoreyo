...
// Configuration - focus on single-question topics
const TARGET_QUESTIONS_PER_TOPIC = 30; // Lower target for faster coverage
const BATCH_SIZE = 10;
const MAX_TOPICS_PER_RUN = 30; // Process more topics per run
...
async function bulkGenerateSingleQuestionTopics() {
  console.log("\n🚀 BULK GENERATION - SINGLE QUESTION TOPICS\n");
  console.log("=".repeat(80));

  // Get topics with exactly 1 question
  ...
  console.log(`\n📊 Found ${singleQuestionTopics.rows.length} topics with only 1 question\n`);
  ...
  for (const row of singleQuestionTopics.rows) {
  ...
  console.log("   2. Run this script again to continue filling single-question topics");
  console.log("=".repeat(80) + "\n");
}

bulkGenerateSingleQuestionTopics().catch(console.error);

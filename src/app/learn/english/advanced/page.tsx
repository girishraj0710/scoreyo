...
import { advancedPathRedesigned } from '@/lib/english-advanced-14';
    ...
    <div className="min-h-screen bg-[#F8FAFC] dark:bg-[#0F1419]">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-slate-900 dark:text-white">
                  {advancedPathRedesigned.name}
                </h1>
                <p className="text-sm text-slate-500 dark:text-slate-400">
                  {advancedPathRedesigned.description}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-4">
              <div className="text-right">
                ...
                <p className="text-lg font-bold text-[#7C3AED]">{advancedPathRedesigned.modules.reduce((acc, m) => acc + m.topics.length, 0)}</p>
              </div>
              <div className="text-right">
                <p className="text-xs text-slate-500 dark:text-slate-400">Est. Time</p>
                <p className="text-lg font-bold text-[#10B981]">{advancedPathRedesigned.estimatedWeeks} weeks</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <main className="max-w-6xl mx-auto px-6 py-8">
        ...
        <div className="space-y-6">
          {advancedPathRedesigned.modules.map((module, moduleIndex) => (
            <div
              key={module.id}
            ...
            >
              ...
              <div className="flex items-center gap-4 mb-6">
                <div className="w-12 h-12 rounded-xl bg-[#7C3AED]/10 flex items-center justify-center">
                  <span className="text-xl font-bold text-[#7C3AED]">{moduleIndex + 1}</span>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {module.topics.map((topic) => (
                  <button
                    key={topic.id}
                    onClick={() => router.push(`/learn/english/advanced/${topic.id}`)}
                    className="group relative p-5 rounded-xl border-2 border-slate-200 dark:border-slate-700 hover:border-[#7C3AED] hover:shadow-lg hover:-translate-y-0.5 transition-all text-left bg-white dark:bg-slate-800"
                  >
                    {/* CEFR Level Badge - Top Right */}
                    <div className="absolute top-4 right-4 px-2 py-0.5 rounded-md bg-[#7C3AED]/10 text-[10px] font-bold text-[#7C3AED]">
                      {topic.cefrLevel}
                    </div>
                    ...
                    <div className="grid grid-cols-[3rem_1fr] gap-3 items-center mb-3">
                      ...
                      {/* Title */}
                      <h3 className="font-semibold text-slate-900 dark:text-white group-hover:text-[#7C3AED] transition-colors text-sm leading-tight pr-10 break-words">
                        {topic.name}
                      </h3>
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      {/* Status Indicator (locked or not started) */}
                      {topic.prerequisite && topic.prerequisite.length > 0 ? (
                        <div title="Complete prerequisites first">
                          <Icons.locked
                            className={`${IconSizes.md} text-amber-500`}
                            aria-label={IconLabels.locked}
                          />
                        </div>
                      ) : (
                        <Icons.notStarted

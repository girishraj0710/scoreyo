...
export default function StudyGuidesPage() {
        ...
        try {
          const sessionRes = await fetch('/api/study/start-session', {
            ...
            body: JSON.stringify({
              subjectId,
              topicId: topicName,
              pathId: examId // Use examId as pathId for study guides
            })
          });
          if (sessionRes.ok) {
            const sessionData = await sessionRes.json();
            setReadingSessionId(sessionData.sessionId);
            setSectionsViewed(new Set()); // Reset sections viewed for new topic
          }
        } catch {
  ...
  // Track section views for time tracking
  useEffect(() => {
    if (activeTOCSection) {
  ...
  // End reading session on unmount or topic change
  useEffect(() => {
    return () => {
  ...
  const handleExamChange = (examId: string) => {
    // End previous session before switching
    if (readingSessionId && sectionsViewed.size > 0 && topicContent) {
      const totalSections = topicContent.content?.sections?.length || 1;
      const completionPct = Math.round((sectionsViewed.size / totalSections) * 100);

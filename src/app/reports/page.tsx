      ...
      <div className="max-w-3xl mx-auto px-4 py-16 text-center">
        <div className=" rounded-2xl p-12 shadow-lg border border-red-200" style={{ background: "var(--card-bg)" }}>
          ...
          <div className="flex gap-3 justify-center">
            ...
            <a href="/dashboard" className="px-6 py-3 bg-slate-100  font-medium rounded-xl hover:" style={{ background: "var(--background)" }} style={{ color: "var(--foreground)" }}>
              {t("dashboard")}
            </a>
          </div>
        </div>
    ...
    return (
      <div className="max-w-3xl mx-auto px-4 py-16 text-center">
        <div className=" rounded-2xl p-12 shadow-lg border border-amber-200" style={{ background: "var(--card-bg)" }}>
          ...
          <p className=" mb-6" style={{ color: "var(--foreground)" }}>{t("reportsProDesc")}</p>
          <div className="flex gap-3 justify-center">
    ...
    return (
      <div className="max-w-3xl mx-auto px-4 py-16 text-center">
        <div className=" rounded-2xl p-12 shadow-lg border " style={{ borderColor: "var(--card-border)" }} style={{ background: "var(--card-bg)" }}>
          ...
          <p className=" mb-6" style={{ color: "var(--foreground)" }}>{t("noReportDataDesc")}</p>
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      <div className="text-center mb-8">
        ...
        <p className="" style={{ color: "var(--foreground)" }}>{t("reportsSubtitle")}</p>
      </div>

      {/* Overview Cards */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <div className=" rounded-xl p-5 shadow-sm border  text-center" style={{ borderColor: "var(--card-border)" }} style={{ background: "var(--card-bg)" }}>
          <div className="text-3xl font-bold text-violet-600">{Number(stats.totalSessions) || 0}</div>
          <div className="text-xs  mt-1" style={{ color: "var(--foreground)" }}>{t("totalQuizzes")}</div>
        </div>
        <div className=" rounded-xl p-5 shadow-sm border  text-center" style={{ borderColor: "var(--card-border)" }} style={{ background: "var(--card-bg)" }}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        ...
        <div className=" rounded-xl p-6 shadow-sm border  flex flex-col h-[345px]" style={{ borderColor: "var(--card-border)" }} style={{ background: "var(--card-bg)" }}>
          ...
          {!safeSubjectBreakdown || safeSubjectBreakdown.length === 0 ? (
            <p className=" text-sm" style={{ color: "var(--foreground)" }}>{t("noExamData")}</p>
          ) : (
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-center">
                        <span className="text-sm font-medium  truncate" style={{ color: "var(--foreground)" }}>
                          ...
                          <span className="text-xs  ml-1" style={{ color: "var(--foreground)" }}>({exam?.name})</span>
                        </span>
        ...
        <div className=" rounded-xl p-6 shadow-sm border  h-[345px]" style={{ borderColor: "var(--card-border)" }} style={{ background: "var(--card-bg)" }}>
              ...
              {[
                ...
                return (
                  <div key={band.band}>
                    ...
                    <div className="w-full  rounded-full h-3" style={{ background: "var(--background)" }}>
      ...
      {safeDailyActivity && safeDailyActivity.length > 0 && (
        <div className=" rounded-xl p-6 shadow-sm border  mb-8" style={{ borderColor: "var(--card-border)" }} style={{ background: "var(--card-bg)" }}>
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold " style={{ color: "var(--foreground)" }}>{t("dailyActivity")}</h3>
            <div className="flex items-center gap-4 text-xs " style={{ color: "var(--foreground)" }}>
          ...
          <div className="flex gap-2">
            ...
            <div className="flex items-center justify-center w-4">
              <span className="text-[10px]  font-medium -rotate-90 whitespace-nowrap" style={{ color: "var(--foreground)" }}>Questions</span>
            </div>
            {/* Y-axis labels */}
            <div className="flex flex-col items-end justify-between text-xs  h-48 py-1 pr-2 border-r " style={{ borderColor: "var(--card-border)" }} style={{ color: "var(--foreground)" }}>
                ...
                return (
                  <div key={idx} className="flex-1 flex flex-col justify-end group h-full">
                    ...
                    >
                      ...
                      <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block  text-white text-xs rounded-lg px-3 py-2 whitespace-nowrap z-20 pointer-events-none shadow-lg" style={{ background: "var(--background)" }}>
                        <div className="font-semibold">{new Date(d.day).toLocaleDateString("en-IN", { day: "numeric", month: "short" })}</div>
                        <div className="" style={{ color: "var(--foreground)" }}>{questions} questions attempted</div>
          ...
          <div className="ml-16">
            ...
            <div className="text-center text-[10px]  font-medium" style={{ color: "var(--foreground)" }}>Date</div>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        ...
        <div className=" rounded-xl p-6 shadow-sm border " style={{ borderColor: "var(--card-border)" }} style={{ background: "var(--card-bg)" }}>
          ...
          {!safeStrongTopics || safeStrongTopics.length === 0 ? (
            <p className=" text-sm" style={{ color: "var(--foreground)" }}>{t("moreQuizzesNeeded")}</p>
          ) : (
            <div className="space-y-2">
              {safeStrongTopics.map((topic: any, idx: number) => {
                ...
                return (
                  <div key={idx} className="flex items-center justify-between p-3  rounded-lg border border-emerald-100" style={{ background: "var(--background)" }}>
                    <div>
                      <div className="text-sm font-medium " style={{ color: "var(--foreground)" }}>{topicName}</div>
                      <div className="text-xs " style={{ color: "var(--foreground)" }}>{exam?.name} | {totalAttempted} Q</div>
                    </div>
                    <div className="text-lg font-bold " style={{ color: "var(--foreground)" }}>{Math.round(masteryScore)}%</div>
                  </div>
                );
              })}
            </div>
                  ...
                  <div key={idx} className="flex items-center justify-between p-3 bg-red-50 rounded-lg border border-red-100">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <a
                        ...
                        className="text-xs text-[#4255FF]  px-3 py-2 rounded hover:bg-[#E8EAFF]" style={{ background: "var(--background)" }}
                      >
                        {t("practice")}
                      </a>
                    </div>
      ...
      {safeAccuracyTrend && safeAccuracyTrend.length > 0 && (
        <div className=" rounded-xl p-6 shadow-sm border  mb-8" style={{ borderColor: "var(--card-border)" }} style={{ background: "var(--card-bg)" }}>
          ...
          <div className="flex gap-2">
            {/* Y-axis title (rotated) */}
            <div className="flex items-center justify-center w-4">
              <span className="text-[10px]  font-medium -rotate-90 whitespace-nowrap" style={{ color: "var(--foreground)" }}>Accuracy</span>
            </div>
            {/* Y-axis labels */}
            <div className="flex flex-col items-end justify-between text-xs  h-48 py-1 pr-2 border-r " style={{ borderColor: "var(--card-border)" }} style={{ color: "var(--foreground)" }}>
              <span>100%</span>
          ...
          <div className="ml-16 mt-2">
            <div className="text-center text-xs  mb-1" style={{ color: "var(--foreground)" }}>{t("last20Quizzes")}</div>
            <div className="text-center text-[10px]  font-medium" style={{ color: "var(--foreground)" }}>Quiz Sequence (Oldest → Newest)</div>
          </div>
        </div>
      )}
      ...
      {safeMockTestHistory && safeMockTestHistory.length > 0 && (
        <div className=" rounded-xl p-6 shadow-sm border  flex flex-col h-[400px]" style={{ borderColor: "var(--card-border)" }} style={{ background: "var(--card-bg)" }}>
          ...
          <div className="space-y-2 overflow-y-auto flex-1 pr-2">
            {safeMockTestHistory.map((test: any, idx: number) => {
              try {
                ...
                return (
                  <div key={idx} className="flex items-center justify-between p-3  rounded-lg border " style={{ background: "var(--background)" }} style={{ borderColor: "var(--card-border)" }}>
                    <div className="flex items-center gap-3">
                      {ExamIcon && <ExamIcon className="w-5 h-5" style={{ color: exam?.color || "#6366f1" }} />}
                    <div>
                      <div className="text-sm font-medium " style={{ color: "var(--foreground)" }}>{exam?.name || test.exam_id}</div>
